...
- Create `this_names`, a vector containing the following strings: “Riad”, “Sabine” and “Nastassja”.
...
this_names <-  c("Riad", "Sabine", "Nastassja") 
```

```{r charliste-check }
gradethis::grade_result(
  pass_if(~ this_names[1] == "Riad"),
  pass_if(~ this_names[2] == "Sabine"),
...
Below is an example of vectors containing Booleans (variables with two states, 'true' and 'false' or 0 and 1). Note that true and false are represented by the keywords `TRUE` and `FALSE` (which are reserved terms in the language). TRUE and FALSE can be replaced by `T` and `F` respectively. However, this last solution is not recommended for lisibility reasons and may, in certain contexts (e.g. writing a library) be considered an error.

```{r vector1, exercise=TRUE}
logic <- c(TRUE, FALSE, TRUE) # or, possibly: logic <- c(T, F, T)
print(logic)
...
- Take the following vector x, test whether the positions are less than 0 and put the result in a variable z. Print z.
...
You can also compare the paired positions of two vectors.
...
The `which()` function returns the positions of a vector for which a condition holds.
...
<div class="exo">
- Store in vector `z` all positions for which the values of `x` and `y` are the same
...
R allows you to generate random numbers (*i.e.* **r**andom) from a wide variety of distributions (normal distribution, uniform distribution, Poisson distribution, Binomial distribution...). 

The code below randomly draws 1000 values from a normal distribution with mean 0 and standard deviation 1. The `head()` function displays the first elements of the vector.
...
**NB:** The `length()` function is used to find out the size/length of a vector.
...
<div class="exo">
- Store in `z` 500 values drawn from a normal distribution with a mean of 2 and a standard deviation of 3. The values should be rounded (`round()`) to two decimal.
...
### Visualize the distributions of the generated values
...
<div class="exo">
Plot the distribution of `x` with `hist()` using increasing values $(30 then 50 then 100) for the breaks parameter. What do you observe?
...
### Exercises on generating random numbers

<div class="exo">

- After setting the random number generator (`set.seed(123)`), create 100 values drawn from a normal distribution with parameters `mean=10, sd=2` and store them in a variable named `rnd`.
...
The `sample()` function allows you to randomly select `size` elements out of a vector `x` (see argument below). Note that in the following example, when the `replace` argument is set to `TRUE`, each drawn element is replaced in `x`before drawing the next element. 
...
<div class="exo">
- Perform 3 random selection of 5 elements with replacement from a vector containing all integers from 1 to 10.
...
learnr::question("In the generated vectors, do you observe any duplicate values?",
  answer("No"),
  answer("Yes", correct = TRUE),
  allow_retry = TRUE 
)
...
<div class="exo">
- Perform 3 random draws of 5 elements **without replacement** from a vector containing all integers from 0 to 10.
...
learnr::question("In each of the generated vectors, do you observe any identical values?",
  answer("Yes"),
...
Mathematical operations can be performed on vectors using standard operators: +, -, \*, \\\ ... To raise to a power, the hat operator (^) is used. These operations will be 'vectorized' (*i.e*. applied successively to all elements of the vector(s)). For those familiar with other languages, this substitutes for 'for' loops and significantly simplifies the syntax.

The vector structure of the language makes iterations over the elements of the vector implicit. The advantage of this vectorization is that it allows you to perform very complex operations in just a few lines of code.
...
<div class="exo">
- Calculate the sum of the squares of the values of `x` and `y` for each position. Store the result in `z`.
...
<div class="exo">
- Given the vector below, store in `z` the number of position that are `-Inf/Inf` (use `is.infinite()`).
...
You can browse all the positions of a vector and apply a function with the `sapply()` function. The function is used by passing a vector as the first argument and the name of a function as the second argument.

For example, you can use the function `class()` (or `is()`) to find out the type of each object in memory. We'll come back to the **apply** family of functions later.
