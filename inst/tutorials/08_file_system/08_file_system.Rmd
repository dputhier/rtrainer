...
```{r exomkdir-solution}
dir.create(file.path(fp, 'input'))
```

```{r exorecap, echo=FALSE, exercise.setup="sapply"}
dir.create(file.path(fp, 'input'), recursive=TRUE, showWarnings=FALSE)
```

<div class="exo">

- Check that the 'input' folder exists by listing the contents of `all_projects` subdirectory using `dir()`.
...
gradethis::grade_result(
  ...
  pass_if(~ all(file.path(fp, 'input')))
)
```
</div>
...
```{r newline_2, exercise=TRUE, exercise.setup="setwd" }
cat("Gene\tA\tB\tC\ngene_1\t1\t2\t3\ngene_2\t4\t5\t6\n", 
