...
url <- "https://zenodo.org/record/8210893/files/elisa_artificial.txt"
# Download
file_path <- file.path(dir_path, "elisa_artificial.txt")
if(!file.exists(file_path)) download.file(url=url, destfile = file_path, quiet = TRUE)
elisa <- read.table(file.path(dir_path, "elisa_artificial.txt"), sep="\t", header=TRUE, row.names=1)
```
...
```{r table, exercise=TRUE, exercise.setup="read_table_setup" }
table(elisa$rows, elisa$columns)
```

These eight plates were produced by 4 experimenters on two different days.

```{r table2, exercise=TRUE, exercise.setup="read_table_setup" }
...
```{r facet_wrap, exercise=TRUE, exercise.setup="read_table_setup", fig.height=6, fig.width=6}
p <- ggplot(data = elisa, 
            mapping = aes(x=value))
p + geom_histogram() + 
...
```{r facet_wrap2, exercise=TRUE, exercise.setup="read_table_setup", fig.height=8, fig.width=6}
p <- ggplot(data = elisa, 
            mapping = aes(x=value))
p + geom_histogram() + 
...
```{r userday_dens, exercise=TRUE, echo=FALSE, fig.height=8, fig.width=6, exercise.setup="read_table_setup"}
```
...
Since each user performed an ELISA experiment on Monday and Friday, we can choose a **two-dimensional faceted representation** with `facet_grid()` (a grid/matrix of facets). Here we will create a facetted diagram with `user` as rows and `day` as columns. Note that `user` and `day` should be wrapped in the `vars()` function. 


```{r facet_grid, exercise=TRUE, exercise.setup="read_table_setup" }
p <- ggplot(data = elisa, 
            mapping = aes(x=value, fill=user))
p + geom_histogram() + 
    facet_grid(rows = vars(user),  cols = vars(day)) +
...
```{r boxplotfacet, exercise=TRUE, echo=FALSE, fig.height=6, fig.width=6, exercise.setup="read_table_setup"}
...
```{r geom_raster, exercise=TRUE, exercise.setup="read_table_setup", fig.height=8, fig.width=6 }
p <- ggplot(data = elisa, 
...
```{r scale_fill_gradientn, exercise=TRUE, exercise.setup="read_table_setup" }
p <- ggplot(data = elisa, 
            mapping = aes(x=columns, y=rows, fill=value))
p + geom_raster() + 
  facet_grid(rows = vars(user), cols= vars(day)) +
...
```{r orderedfactor, exercise=TRUE, exercise.setup="read_table_setup", fig.height=8, fig.width=6}
___
p <- ggplot(data = elisa, 
            mapping = aes(x=columns, y=rows, fill=value))
p + geom_raster() + 
...
gradethis::grade_result_strict(
  pass_if(~ rlang::quo_get_expr(p$mapping$x) == "seqid"),
...
```{r txgenomicsize, exercise=TRUE , exercise.setup="txgenomicsizeprep", fig.height=6, fig.width=4}
...
gradethis::grade_result_strict(
  pass_if(~ rlang::quo_get_expr(p$mapping$x) == "tx_genomic_size_log10"),
...
Transform the `nb_exons` column into its logarithm and place the result in the `nb_exons_log10` column. What can you say about the number of exons (`nb_exons_log10`) for transcripts based on "gene_biotype"? Use a *boxplot* to present this information.
...
```{r nbexons-check }
  gradethis::grade_result_strict(
    pass_if(~ rlang::quo_get_expr(p$mapping$y) == "nb_exons_log10"),
    pass_if(~ rlang::quo_get_expr(p$mapping$x) == "gene_biotype"),
...
gradethis::grade_result_strict(
  pass_if(~ rlang::quo_get_expr(p$mapping$x) == "seqid"),
  pass_if(~ rlang::quo_get_expr(p$mapping$fill) == "gene_biotype"),
  pass_if(~ class(p$layers[[1]]$geom)[1] == "GeomBar")
