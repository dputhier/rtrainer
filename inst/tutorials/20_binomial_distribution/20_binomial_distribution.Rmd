...
title: "The binomial distribution and (r)GREAT functional enrichment tool"
output:
  learnr::tutorial:
    includes:
...
# if the result file does not exist yet
file_path <- file.path(dir_path, "rGREAT_tbl.rds")
...
DT::datatable(tbl[["GO Biological Process"]], 
...
```{r exercise-great-load5, exercise=TRUE, exercise.setup="exercise-great-load4", fig.height=5, fig.width=5 }
rGREAT::plotVolcano(job, ontology = "GO Biological Process")
```
...
The rGREAT::plotVolcano() does not allow to **label specific points**. However, we can extract the data and use `ggplot2` and `ggrepel` to label specific points of interest. Here we will **label points with `abs(log2 fold-enrichment)` greater than 1 and `-log10(p-value)` greater than 10 (to restrict to highly enriched terms)**.

```{r exercise-great-load6, exercise=TRUE, exercise.setup="exercise-great-load5", fig.height=5, fig.width=5 }
library(ggplot2)
...
Go back to the rGREAT table of results. **Search for term "embryonic placenta development" (GO:0001892)**. Using `pbinom()`re-compute the **binomial raw p-value** : 
