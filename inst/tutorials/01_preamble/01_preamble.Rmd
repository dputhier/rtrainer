...
<div class="exo">
Below we use the `data()` function to load the `InsectSprays` dataset from the `datasets` package. 

- Use the `class()` function to determine the type of object stored in `InsectSprays` and assign this to variable `z`.
...
```{r is-solution}
data("InsectSprays", package = "datasets")
z <- class(InsectSprays)
```


```{r is-check}
gradethis::grade_result_strict(
  pass_if(~ z[1] == "data.frame")
)
```
