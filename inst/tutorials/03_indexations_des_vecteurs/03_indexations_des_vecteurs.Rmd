...
Indexing means retrieving certain elements from an object. The indexing operator is the bracket '[]', as in many other languages. It is used in the following form:
...
Targeted positions (target_positions) will take the form of:
  
- A vector of indices (*i.e* positions). Positions are counted from $1$ to $n$ (with $n$ the size of the vector).
- Or a logical vector (true/false).
- Or the names of targeted positions in the vector-these are named.
...
The argument to the indexing operator is a set of positions in numerical format. 

**NB:** Indexing in R is **"one-based "**: **the first position is accessed with index value 1** (unlike Python, for which indexing is **"zero-based "**, the first position is accessed with value 0).
...
We've seen that you can perform logical operations with the "&" and "|" operators. We can therefore also use these operators to test the positions of a vector on the basis of another vector (typically if they both have the same size). 
...
Using indexing, you can select the points to be highlighted. For example, you can easily color positive points on the x-axis and y-axis. To do this, use the `points()` function, which overlay points on an existing diagram. 
...
Complete the code to display all points within a circle of center 0,0 and radius 2 in red (remember Pythagoras' theorem!). 
...
The positions of a vector can be named. If they are, positions can be extracted by passing their names to the indexing operator ('[') in the form of a vector object.

For example, a vector could contain the expression of genes analyzed by QPCR. Each position would then be the gene symbol.
...
The `grep()` function can be used to search for genes that verify a given regular expression. Regular expressions can be used to **search for patterns in character strings**. The syntax provides a **set of operators to describe the pattern being searched for**.
...
The `grep()` function returns **the positions verifying a regular expression** (*i.e* containing a particular string motif). For example, here, `grep()` can be used to search for all positions containing a gene whose name begins (\^) with “CD”.
