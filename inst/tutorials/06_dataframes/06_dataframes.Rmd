...
- Les objets *data.frame* contenant potentiellement autre chose que des valeurs numérique la fonction *hist()* ne les acceptera pas par défaut. Convertisez le *data.frame* suivant en matrice (objet $mat$) et passez cette matrice à la fonction *hist()* pour construire l'histogramme correspondant.  
...
```{r dataframe2, exercise=TRUE, setup.exercice="dataframe1"}
...
Tapez `chickwts$` et pressez la touche <TAB> (tabulation) pour afficher le contenu de la colonne *feed*. Faites de même pour la colonne *weight*.

```{r dataframe3, exercise=TRUE, setup.exercice="dataframe1"}
...
```{r idx1, exercise=TRUE, setup.exercice="dataframe1"}
chickwts[chickwts$weight > 400, ]
```

On encore:

```{r idx2, exercise=TRUE, setup.exercice="dataframe1"}
chickwts[chickwts$weight > 400 & chickwts$feed == "casein",  ]
```
