...
Suppose that, at a campus entrance, the **observed mean number of cars arriving during a 10-minute interval is 5.3**. We denote this expected value by \(\lambda = 5.3\).

We would like to approximate the probability \(P(X = k)\) (for example, \(P(X = 10)\)) using a **Binomial distribution**. To do so, can change the time scale so that we create \(n\) **very small and equal time intervals**. Each sub-interval is treated as a **Bernoulli trial** with two possible outcomes:

- **success**: exactly one car arrives during the sub-interval  
- **failure**: no car arrives  

If the **intervals are sufficiently small**, the probability of **observing more than one car in a single sub-interval is negligible**. Under this assumption, the  number of cars \(X\) arriving at the campus entrance can be modeled as a Binomial random variable with parameters \(n\) and \(p\).

Recall that the expected value of a Binomial distribution is
\[
E[X] = np.
\]

Since the mean number of cars is \(\lambda\), we can write:
\[
np = \lambda,
\]
which gives
...
Substituting this expression into the Binomial probability mass function, we obtain
\[
\begin{aligned}
P(X = k)
...
We consider the situation where the number of trials \(n\) becomes **very large**, and thus the probability of success \(p\) becomes **very small**.
This corresponds to measuring time using extremely fine intervals (seconds, microseconds, nanoseconds, …). In such small intervals, the probability of observing more than one car is essentially zero, and thus the binomial model is appropriate.
:::

As $n$ gets high the number of success stay constant and thus **for large** $n$ and small $k$:

$n \approx (n-1) \approx(n-2) \approx ... \approx (n-k+1)$
...
**As** $n \to \infty$:  $\left(1 - \frac{\lambda}{n}\right)^{-k} \to 1$ (since $\frac{\lambda}{n} \to 0$)
...
Thus, **as the number of trials $n$ approaches infinity and the probability of success $p$ becomes infinitesimally small**, the Binomial distribution converges to the Poisson distribution. This behavior simplifies the mathematical framework. Indeed, with **based solely on the average rate of occurrence**, $\lambda$, the Poisson distribution allows us to infer the entire probability distribution of event counts, whether in time, space, or other intervals. One important property of the Poisson distribution is that its **mean and variance are both equal to** $\lambda$.
...
sliderInput("lambda", "Average rate (λ):",
...
output$poissonPlot <- renderPlot({
  ...
  ggplot(data, aes(x = k, y = probabilities)) +
    ...
    labs(title = paste("Poisson Distribution (λ =", input$lambda, ")"),
...
1.  **`dpois(x, lambda)`**: Probability Mass Function (PMF)
    ...
    -   Example: `dpois(3, lambda = 2)` gives P(X=3) when λ=2
2.  **`ppois(q, lambda)`**: Cumulative Distribution Function (CDF)
    -   Returns the **cumulative probability P(X ≤ q)**
    -   Use **`lower.tail = FALSE` for P(X \> q)**
    -   Example: `ppois(5, lambda = 3)` gives P(X ≤ 5) when λ=3
3.  **`qpois(p, lambda)`**: Quantile Function
    -   Returns the smallest integer `q` such that P(X ≤ q) ≥ p
...
5.  Calculate the probability of observing 4 events or more (the p-value of 4) when 𝜆=3 . Store your result in a variable called prob_4_or_more
...
library(png)
download.file("https://zenodo.org/records/18164316/files/example_chip.png", 
              destfile = file.path(fs::path_home(), ".rtrainer", "example_chip.png"))
pp <- readPNG(file.path(fs::path_home(), ".rtrainer", "example_chip.png"))
plot.new() 
...
2. For each window, use the input count as λ (local Poisson parameter)
