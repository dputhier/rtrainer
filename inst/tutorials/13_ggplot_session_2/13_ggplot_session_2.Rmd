...
In this histogram, we used a class interval of \$1000 to group the values of the `price` variable (\$0-\$1000, \$1000-\$2000, \$2000-\$3000, ...). The plot displays the frequency of diamond prices within each class interval.
...
- Complete the following code so that the coloring is associated with the quality of the cut (`cut`) of the diamonds. Assign colors of your choice using `scale_fill_manual()`.
...
The density plot is another way to visualize the distribution of values in a numeric variable. It can be created using the `geom_density()` function. The `geom_density()` function displays a distribution model that aims to closely fit the histogram data. However, it is a model, and the `adjust` parameter of this function can significantly influence the representation. Depending on the value of `adjust`, the model may tend to be over- or under-fitted.
...
- Complete the code to produce a plot where the densities for the different cuts (`cut`) are represented with a unique border color for each. Apply your own colors.
...
- Complete the code to produce a plot where the densities for the different cuts (`cut`) are represented with a unique fill color for each. Apply your own colors.
...
This can be resolved by using `geom_jitter()` (instead of `geom_point()`), which adds **a bit of noise to the x-axis** to offset the points from the vertical lines.
