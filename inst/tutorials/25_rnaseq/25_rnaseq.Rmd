...
The RNA-Seq dataset we will use in this practical has been produced by **Gierliński *et al*.** ([PMID:26206307](https://pubmed.ncbi.nlm.nih.gov/26206307/)).  
...
Given a matrix  with $p$ columns (samples) and $n$ rows (genes) **DESeq2 estimates the size factors** as follows: 
...
Let $K_{g,j}$ be the count for gene $g$ in sample $j$. The geometric mean for gene $g$ across all samples is computed as:
  $$(\prod_{j=1}^p K_{g,j})^{1/p} $$
The scaling factor for sample $j$ is thus obtained as:


  $$sf_{j} = median(\frac{K_{g,j}}{(\prod_{j=1}^p K_{g,j})^{1/p}}) $$
An implementation of this method is given below for illustration purposes.
...
- **log2FoldChange**: The log2 fold change between the two conditions (WT
  vs KO).
...
fluidPage(
  h3("Linear Fold Change vs log2 Fold Change"),

  p(
    "Use this interactive example to understand how linear fold changes ",
    "are transformed into log\u2082 fold changes, and why the log\u2082 scale ",
    "provides a symmetric and more interpretable representation of gene ",
  ...
  fluidRow(
    column(
      6,
      h4("Linear scale"),
      plotOutput("linearPlot", height = 260)
    ),
    column(
      6,
      h4("log2 scale"),
      plotOutput("logPlot", height = 260)
    )
  ),
...
fold_change <- reactive({
  input$test_expr / input$ref_expr
})
...
output$logPlot <- renderPlot({
  ...
  ggplot(df, aes(condition, log2expr, fill = condition)) +
    geom_col(width = 0.6) +
    theme_bw() +
