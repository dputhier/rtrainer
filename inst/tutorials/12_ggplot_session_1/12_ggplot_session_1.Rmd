...
path <- file.path(system.file(package="rtrainer"), "tutorials", 
                  "12_ggplot_session_1", "img", "ggbasics.png")
pp <- readPNG(path)
plot.new() 
...
str(diamonds)
```
Note that the `diamonds` object could be a `data.frame`. In fact, it is a `tibble`, which is a type of `data.frame` where the columns are slightly more typed. For example, it distinguishes between real numbers (`<dbl>`) and integers (`<int>`). However, you can easily convert this `tibble` into a `data.frame` to obtain an object you are already familiar with.
...
- *(iii)* Defining **the variables/columns of the data.frame to be analyzed** and passing them to the `aes()` function.

For a scatterplot, two aesthetics are mandatory: $x$ (x-axis) and $y$ (y-axis). The $x$ and $y$ variables are typically numeric. Here, we will use the `carat` and `price` columns, both of which are numeric.
...
# Display the names of the colors available in R
color_avail <- colors()
