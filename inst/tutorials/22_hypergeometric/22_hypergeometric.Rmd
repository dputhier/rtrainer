...
```{r fisher, exercise=TRUE, exercise.setup="reload"}
# Parameters for the hypergeometric test
m <- ___
n <- ___
k <- ___
...
mat <- matrix(c(a, b, c, d), nrow=2, byrows=TRUE)  
...
```{r fisher-check }
gradethis::grade_result(
  pass_if(~ abs(pval_fisher - phyper(16-1, 278, 18888-278, 47, lower.tail = FALSE)) < 1e-10)
)
```
