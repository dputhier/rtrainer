...
library(png)
library(RCurl)
pp <- readPNG(RCurl::getURLContent("https://zenodo.org/records/18144510/files/urn.png"))
plot.new() 
...
library(png)
pp <- readPNG(RCurl::getURLContent("https://dputhier.github.io/scigenex/articles/usage_files/figure-html/plot_ggheatmap-1.png"))
plot.new() 
rasterImage(pp,0,0,1,1)
```
...
# Load AnnotationDbi package for selecting gene annotations
library(AnnotationDbi)

# Get the entrez IDs for a given gene set (here for module 1)
query_entrezid <- AnnotationDbi::select(get("org.Hs.eg.db"),
                                        keys = gene_set,
                                        columns = c("ENTREZID", "SYMBOL"),
                                        keytype = "SYMBOL")

# Perform GO enrichment analysis using clusterProfiler
enrich_res <- clusterProfiler::enrichGO(query_entrezid[,"ENTREZID"],
...
### Checking we agree with the results
...
```{r exohypergeo, exercise=TRUE, exercise.setup="reload"}
...
gradethis::grade_result(
  pass_if(~ abs(pval_hypergeo - phyper(16-1, 278, 18888-278, 47, lower.tail = FALSE)) < 1e-10)
)
```
...
```{r rich_factor, exercise=TRUE, exercise.setup="reload"}
rich_factor <- 
```
...
```{r FoldEnrichment, exercise=TRUE, exercise.setup="reload"}
fold_enrichment <- ___
```
...
- $\mathbb{E}[X]$: expected count ($k \times \frac{m}{m+n}$)
- $\mathrm{Var}(X)$: variance of the hypergeometric distribution ($k \cdot \frac{m}{m+n} \cdot (1 - \frac{m}{m+n}) \cdot \frac{m+n-k}{m+n-1}$
...
```{r zscore, exercise=TRUE, exercise.setup="reload"}
# Parameters
m <- 278
n <- 18888-278
k <- 47
...
```{r zscore-solution, exercise=TRUE, exercise.setup="reload"}
m <- 278
n <- 18888-278
k <- 47
x <- 16
...
<div class="exo">
Use R code to create a barplot of the top 10 enriched GO terms for gene module 6 (results are stored in `enrich_res`). Store the plot in `barplot_enrich_exo`.
...
library(enrichplot)
enrichplot::barplot.enrichResult(enrich_res, showCategory = 10) 
