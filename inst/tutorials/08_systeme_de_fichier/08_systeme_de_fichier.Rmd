...
Ensuite on peut créer les dossiers correspondants. Comme il y en a plusieurs et que **dir.create()** n'en accepte qu'un, on va **'appliquer'** (avec **sapply**) à toutes les positions du vecteur **fp** la fonction **dir.create()**. Par défaut **dir.create()** n'accepte pas de créer un dossier enfant si le dossier parent n'existe pas (ici "projects"). On le force à accepter de le faire en indiquant **recursive=TRUE**. Comme pour l'ensemble des fonction de la **famille apply**, les arguments supplémentaires de la fonction appelée (**dir.create**) son passés à la fin (**recursive=TRUE, showWarnings=FALSE**).
