...
In R, **factors** are categorical variables that can be created from character strings or numerical vectors. The **factor** is a central object in R as it will allow, for instance, to:

- Create diagrams (*e.g* distributions, boxplot...)
- Compute statistics (*e.g* mean, variance, t-test...) according to categories
- ...

As an example, you may be interested to analyze values according to any membership. For example, below, we create an ordered vector containing the treatments received by 500 patients. 
...
The contingency table can also be represented graphically with basic R commands. The areas then correspond to the numbers.

```{r exo6b, exercise=TRUE, exercise.setup="exo5" }
mosaicplot(table(ttmt, gender), color = c("salmon", "olivedrab"), main="Mosaic plot")
```

The areas then correspond to the size of each category.
...
An ordinal variable is rather close to a categorical variable. The only difference is that ordinal variables are ordered (e.g. education level: "high school", "BS","MS","PhD"; rating: "like", "neutral", "dislike"...).

### Creating ordinal variables

For example, `age` could be naturally converted to an ordinal variable using the `factor()` function (which contains additional arguments compared to `as.factor()`). 
...
A factor can be used to perform operations on a vector according to categories. For example, you can apply functions in each category with `tapply()`. Here we give the example of calculating the mean (`mean()`) and visualizing the distributions of responses (*infection*) in the 3 classes of patients (*i.e.* having received the three treatments). 
...
But we can also analyze the effect (`~`) of treatment and (`+`) gender.  

```{r exo9b, exercise=TRUE, exercise.setup="exo7" } 
# We change the graphic parameters (par())
# to increase the size of my margin (mar) on the left, 
# because the class names are long.
par(mar=c(1,7,1,1))
boxplot(infection ~ ttmt + gender, horizontal = TRUE, las=1, ylab="")
# We add a grid for readability.
grid()
```

## Creating factors with the cut() function

The *cut()* function divides the input numeric vector into intervals and encodes the values of the input vector according to the intervals to which they belong. This allows, when needed, to transform a numerical variable into a set of categories.
