...
- Etant donné la categorie age comptez le nombre d'individus dans les différentes modalités à travers les facteurs ttmt, gender et age.

**NB:** Le premier argument de table (*cf* *help(table)*) est '...' ce qui indique qu'on peut passer autant de facteur que l'on souhaite en argument.
...
Il est possible de créer des variables catégorielles ordinales dans lesquels il existe une relation d'ordre entre les catégories possibles. Par exemple le traitement 2 est plus puissant que le traitement 1 (e.g. dose différentes). 
...
Ces facteurs sont extrêmement intéressants aussi pour des opérations graphiques. Par exemple, ci-dessous on réalise des boites à moustache en fonction des catégories de patient. 
...
Maintenant qu'on a crée des classes de niveau d'infection, on pourra créer le tableau de contingence correspondante. 
...
Nous reverrons l'utilisation de cet objet *factor* plus en détail par suite.
